...
const rootTsConfigPath = path.resolve(process.cwd(), "./tsconfig_root.json");
const localTsConfigPath = path.resolve(process.cwd(), "./tsconfig.json");

// Check the environment variable
if (process.env.IS_VERCEL_ENV !== "true") {
  console.log(
    "Skipping preinstall script because IS_VERCEL_ENV is not set to 'true'."
  );
  process.exit(0);
}
...
fs.readFile("./package.json", "utf8", function (err, data) {
  ...
  Object.keys(packageJson.dependencies).forEach((pkg) => {
    ...
    if (
      ...
      const latestVersion = execSync(`npm show ${pkg}@dev-v2 version`, {
        encoding: "utf8",
      }).trim();
  ...
  fs.readFile(localTsConfigPath, "utf8", function (err, localData) {
    ...
    let mergedTsConfig = {
      ...
      include: [
        ...
        ...(localTsConfig.include || []),
      ],
      exclude: [
        ...(rootTsConfig.exclude || []),
        ...(localTsConfig.exclude || []),
      ],
    };
